...
const fetchSystemOverview = async () => {
  try {
    const res = await client.get('/admin/systems/dawah-academy');
...
const fetchHealth = async () => {
  try {
    const res = await client.get('/admin/systems/dawah-academy/health');
...
const fetchMetrics = async () => {
  try {
    const res = await client.get('/admin/systems/dawah-academy/metrics');
...
const fetchIntegrations = async () => {
  try {
    const res = await client.get('/admin/systems/dawah-academy/integrations');
...
const fetchConfig = async () => {
  try {
    const res = await client.get('/admin/systems/dawah-academy/config');
...
const loadAllData = async (forceRefresh = false) => {
  ...
  try {
    ...
    error.value = 'Failed to fetch some Dawah Academy diagnostics. Please verify connection.';
  } finally {
    loading.value = false;
  }
};
...
const handleSaveConfig = async () => {
  ...
  try {
    const res = await client.put('/admin/systems/dawah-academy/config', configData.value);
    if (res.data.success) {
    ...
    <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-6 bg-white border border-neutral-ivory rounded-3xl p-6 shadow-soft relative overflow-hidden">
      <div class="space-y-1 z-10">
        <div class="flex items-center gap-3">
          <h1 class="text-3xl font-display font-black text-primary">Dawah Academy Portal</h1>
          <span 
            v-if="systemStatus" 
            :class="[
        ...
        <p class="text-xs text-neutral-muted">LMS platform diagnostics, active courses oversight, student progress audits, and academy configurations deck.</p>
      </div>
      ...
      <div class="lg:col-span-9">
        <div class="bg-white border border-neutral-ivory rounded-3xl p-6 sm:p-8 shadow-soft min-h-[400px]">
          ...
          <div v-if="activeTab === 'overview'" class="space-y-6">
            ...
            <p class="text-xs text-neutral-muted">Dawah Academy learning activity, server response metrics, and course logs.</p>

            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div class="p-5 border border-neutral-ivory rounded-2xl bg-neutral-background/20 relative overflow-hidden">
                <h4 class="text-[10px] font-black uppercase tracking-wider text-neutral-muted">LMS Latency</h4>
                <p class="text-2xl font-semibold text-primary mt-2">{{ healthData?.api?.avg_latency_ms ?? 0 }} ms</p>
                <div class="text-[10px] text-neutral-muted mt-2">Average response speed for learning api</div>
              </div>
              <div class="p-5 border border-neutral-ivory rounded-2xl bg-neutral-background/20 relative overflow-hidden">
                ...
                <div class="text-[10px] text-neutral-muted mt-2">Database query response times</div>
              </div>
              <div class="p-5 border border-neutral-ivory rounded-2xl bg-neutral-background/20 relative overflow-hidden">
                <h4 class="text-[10px] font-black uppercase tracking-wider text-neutral-muted">Gamification Engine</h4>
                <p class="text-2xl font-semibold text-primary mt-2">Active</p>
                <div class="text-[10px] text-neutral-muted mt-2">Badges, Milestones, and Certs Awarder</div>
              </div>
            </div>
              ...
              <div class="border border-neutral-ivory rounded-2xl overflow-hidden shadow-sm">
                <table class="w-full text-xs text-left border-collapse">
                  ...
                  <tbody class="divide-y divide-neutral-ivory/40">
                    <tr>
                      <td class="p-4 font-bold">LMS Core Engine</td>
                      <td class="p-4">Enrollments, progress tracker, and certificates validation</td>
                      <td class="p-4">
                    ...
                    <tr>
                      <td class="p-4 font-bold">Peer Discussions</td>
                      <td class="p-4">Collaborative channels moderation and threads activity</td>
                      <td class="p-4">
                        <span :class="['px-2 py-0.5 rounded text-[10px] font-bold border', healthData?.discussions?.status === 'operational' ? 'bg-emerald-50 text-emerald-700 border-emerald-100' : 'bg-amber-50 text-amber-700 border-amber-100']">
                          {{ healthData?.discussions?.status === 'operational' ? 'Operational' : 'Moderation Warning' }}
                        </span>
                      </td>
                    </tr>
                    <tr>
                      <td class="p-4 font-bold">Onboarding Sanctuary</td>
                      <td class="p-4">New student profile hydration and intentions setter flow</td>
                      <td class="p-4">
                        <span class="px-2 py-0.5 rounded text-[10px] font-bold border bg-emerald-50 text-emerald-700 border-emerald-100">
                          Enabled
                        </span>
          ...
          <div v-if="activeTab === 'health'" class="space-y-6">
            ...
            <p class="text-xs text-neutral-muted">Continuous background checks across learning systems and collaborative environments.</p>
            ...
            <div v-if="healthData" class="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div class="p-5 border border-neutral-ivory rounded-2xl space-y-3">
                <div class="flex justify-between items-center">
                  <span class="text-xs font-bold text-neutral-black">Academy API Gateway</span>
              ...
              <div class="p-5 border border-neutral-ivory rounded-2xl space-y-3">
                ...
                <div class="text-xs text-neutral-muted space-y-1">
                  ...
                  <div>LMS Schemas Status: <span class="font-bold text-neutral-black">Synchronized</span></div>
                </div>
              </div>
              ...
              <div class="p-5 border border-neutral-ivory rounded-2xl space-y-3">
                <div class="flex justify-between items-center">
                  <span class="text-xs font-bold text-neutral-black">Discussion Forums</span>
                  <span :class="['px-2 py-0.5 rounded text-[10px] font-bold uppercase', healthData.discussions.status === 'operational' ? 'bg-emerald-50 text-emerald-700' : 'bg-amber-50 text-amber-700']">
                    {{ healthData.discussions.status }}
                  </span>
                </div>
                <div class="text-xs text-neutral-muted space-y-1">
                  <div>Open Reports: <span class="font-bold text-red-600">{{ healthData.discussions.open_reports }}</span></div>
                  <div>Active Threads/Posts: <span class="font-bold text-neutral-black">{{ healthData.discussions.threads }} / {{ healthData.discussions.posts }}</span></div>
                </div>
              </div>
              ...
              <div class="p-5 border border-neutral-ivory rounded-2xl space-y-3">
                <div class="flex justify-between items-center">
                  <span class="text-xs font-bold text-neutral-black">Cache Registry</span>
                ...
                <div class="text-xs text-neutral-muted space-y-1">
                  ...
                  <div>Status Check: <span class="font-bold text-neutral-black">Healthy</span></div>
                </div>
              </div>
            </div>
          </div>
          ...
          <div v-if="activeTab === 'metrics'" class="space-y-6">
            <h2 class="text-xl font-display font-bold text-primary">LMS & Academy Metrics</h2>
            <p class="text-xs text-neutral-muted">Academic courses catalog counts, total achievements and student enrollment details.</p>
            ...
            <div v-if="metricsData" class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
              <div class="p-5 border border-neutral-ivory rounded-2xl shadow-soft">
                <div class="text-[10px] font-black uppercase text-neutral-muted tracking-wider">Total Courses</div>
                <div class="text-3xl font-bold text-primary mt-2">{{ metricsData.courses }}</div>
                <div class="text-[10px] text-neutral-muted mt-2">Active courses configured in catalog</div>
              </div>
              <div class="p-5 border border-neutral-ivory rounded-2xl shadow-soft">
                <div class="text-[10px] font-black uppercase text-neutral-muted tracking-wider">Modules & Lessons</div>
                <div class="text-3xl font-bold text-primary mt-2">{{ metricsData.modules }} / {{ metricsData.lessons }}</div>
                <div class="text-[10px] text-neutral-muted mt-2">Learning modules & individual lessons</div>
              </div>
              <div class="p-5 border border-neutral-ivory rounded-2xl shadow-soft">
                <div class="text-[10px] font-black uppercase text-neutral-muted tracking-wider">Quizzes & Questions</div>
                <div class="text-3xl font-bold text-primary mt-2">{{ metricsData.quizzes }} / {{ metricsData.questions }}</div>
                <div class="text-[10px] text-neutral-muted mt-2">Course evaluations & question pool</div>
              </div>
              <div class="p-5 border border-neutral-ivory rounded-2xl shadow-soft">
                <div class="text-[10px] font-black uppercase text-neutral-muted tracking-wider">Achievements & Badges</div>
                <div class="text-3xl font-bold text-primary mt-2">{{ metricsData.achievements }} / {{ metricsData.badges }}</div>
                <div class="text-[10px] text-neutral-muted mt-2">Interactive badges, XP and awards checklist</div>
              </div>
              <div class="p-5 border border-neutral-ivory rounded-2xl shadow-soft">
                <div class="text-[10px] font-black uppercase text-neutral-muted tracking-wider">Total Enrollments</div>
                <div class="text-3xl font-bold text-primary mt-2">{{ metricsData.enrollments }}</div>
                <div class="text-[10px] text-neutral-muted mt-2">Active learners: {{ metricsData.active_enrollments }} • Done: {{ metricsData.completed_enrollments }}</div>
              </div>
              <div class="p-5 border border-neutral-ivory rounded-2xl shadow-soft bg-neutral-background/30 font-mono">
                <div class="text-[10px] font-black uppercase text-neutral-muted tracking-wider">Quiz Attempts Performance</div>
                <div class="text-3xl font-bold text-emerald-700 mt-2">{{ metricsData.average_quiz_score }}%</div>
                <div class="text-[10px] text-neutral-muted mt-2">Attempts: {{ metricsData.quiz_attempts }} • Passed: {{ metricsData.passing_quiz_attempts }}</div>
              </div>
            </div>
          </div>
          ...
          <div v-if="activeTab === 'integrations'" class="space-y-6">
            ...
            <p class="text-xs text-neutral-muted">Academy companion engines and gamification profiles.</p>
            ...
            <div v-if="integrationData" class="space-y-6">
              ...
              <div class="border border-neutral-ivory rounded-3xl p-6 space-y-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 text-xs">
                  <div class="space-y-2">
                    <div>Service Status: <span class="font-bold text-neutral-black">{{ integrationData.ai_mentor.status }}</span></div>
                    <div>Default Engine: <span class="font-bold text-neutral-black capitalize">{{ integrationData.ai_mentor.service }}</span></div>
                  </div>
                  <div class="space-y-2">
                    <div>Active sessions (last 30m): <span class="font-mono text-neutral-black">{{ integrationData.ai_mentor.active_sessions }}</span></div>
                  </div>
                </div>
              </div>

              <!-- Gamification Dispatch -->
              <div class="border border-neutral-ivory rounded-3xl p-6 space-y-4">
                <div class="flex justify-between items-center border-b border-neutral-ivory/60 pb-3">
                  <h3 class="text-sm font-bold text-primary flex items-center gap-2">
                    <span class="h-2 w-2 rounded-full bg-emerald-500"></span>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 text-xs">
                  <div class="space-y-2">
                    <div>Engine Status: <span class="font-bold text-neutral-black">{{ integrationData.gamification.status }}</span></div>
                  </div>
                  <div class="space-y-2">
                    <div>Active features: <span class="font-bold text-neutral-black">{{ integrationData.gamification.features.join(', ') }}</span></div>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div v-if="activeTab === 'config'" class="space-y-6">
            <div class="flex justify-between items-center border-b border-neutral-ivory/60 pb-3">
              <div>
                <h2 class="text-xl font-display font-bold text-primary">Dawah Academy configurations</h2>
                <p class="text-xs text-neutral-muted">Centralized settings deck for courses default thresholds.</p>
              </div>
            </div>
            ...
            <form v-if="configData" @submit.prevent="handleSaveConfig" class="space-y-6 text-xs">
              <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="space-y-3 p-5 border border-neutral-ivory rounded-2xl">
                  <h3 class="text-xs font-bold text-primary">Evaluations & Quizzes</h3>
                  <div class="space-y-1.5">
                    <label class="font-bold text-neutral-muted block">Passing Score - Quizzes (%)</label>
                    <input type="number" v-model.number="configData.course_passing_score" :disabled="!isSuperAdmin" class="w-full bg-white border border-neutral-ivory rounded-xl p-2.5 text-neutral-black" />
                  </div>
                  <div class="space-y-1.5">
                    <label class="font-bold text-neutral-muted block">Max Attempts per Quiz</label>
                    <input type="number" v-model.number="configData.max_quiz_attempts" :disabled="!isSuperAdmin" class="w-full bg-white border border-neutral-ivory rounded-xl p-2.5 text-neutral-black" />
                  </div>
                </div>

                <!-- Gamification limits -->
                <div class="space-y-3 p-5 border border-neutral-ivory rounded-2xl">
                  <h3 class="text-xs font-bold text-primary">Student defaults & Limits</h3>
                  <div class="space-y-1.5">
                    <label class="font-bold text-neutral-muted block">Daily XP limit per volunteer</label>
                    <input type="number" v-model.number="configData.daily_xp_limit" :disabled="!isSuperAdmin" class="w-full bg-white border border-neutral-ivory rounded-xl p-2.5 text-neutral-black" />
                  </div>
                  <div class="space-y-1.5 flex justify-between items-center p-2 bg-neutral-background/10 rounded-xl mt-3">
                    <span class="font-bold text-neutral-muted">Enable Gamification Modules</span>
                    <input type="checkbox" v-model="configData.gamification_enabled" :disabled="!isSuperAdmin" class="h-4.5 w-4.5 rounded cursor-pointer" />
                  </div>
                </div>

                <!-- Notifications -->
                <div class="space-y-3 p-5 border border-neutral-ivory rounded-2xl md:col-span-2">
                  <h3 class="text-xs font-bold text-primary">Communications & Settings</h3>
                  <div class="space-y-1.5">
                  ...
                  <div class="space-y-1.5 flex justify-between items-center p-2 bg-neutral-background/10 rounded-xl mt-3">
                    <span class="font-bold text-neutral-muted">Enable Classrooms Email Notifications</span>
                    <input type="checkbox" v-model="configData.email_notifications" :disabled="!isSuperAdmin" class="h-4.5 w-4.5 rounded cursor-pointer" />
                  </div>
                </div>
              </div>
