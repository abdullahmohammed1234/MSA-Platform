...
const handleSubmit = () => {
  ...
  console.log('Signed up with:', email.value);
  email.value = '';
...
<template>
  <footer class="bg-primary text-white pt-20 pb-10 overflow-hidden relative border-t border-white/10">
    ...
    <div class="max-w-7xl mx-auto px-6 md:px-12 relative z-10">
          ...
          <div class="bg-primary-dark/20 border border-white/10 rounded-3xl p-8 md:p-10">
            ...
            <form @submit.prevent="handleSubmit" class="flex flex-col sm:flex-row gap-3">
              <input 
                ...
                v-model="email"
                required
                class="flex-grow bg-white/5 border border-white/10 text-white px-6 py-4 rounded-2xl focus:outline-none focus:border-secondary/50 focus:bg-white/10 transition-all placeholder:text-white/30"
              />
              <button 
                type="submit"
                class="bg-secondary text-white px-8 py-4 rounded-2xl font-bold uppercase text-[11px] tracking-widest hover:bg-secondary-light hover:shadow-premium transition-all flex items-center justify-center gap-2 cursor-pointer"
              >
      ...
      <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-12 py-16 border-b border-white/10">
        <div class="space-y-6">
        ...
        <div class="col-span-2 md:col-span-4 lg:col-span-1 space-y-8">
          <div class="space-y-4">
            <h5 class="text-accent-gold text-[10px] uppercase tracking-[0.3em] font-black">Our Partners</h5>
            <div class="grid grid-cols-3 lg:grid-cols-1 gap-4">
              <div class="h-12 bg-white/5 rounded-xl border border-white/5 flex items-center justify-center grayscale opacity-30 hover:grayscale-0 hover:opacity-100 transition-all cursor-pointer group">
                <Globe :size="16" class="group-hover:text-accent-gold" />
              </div>
              <div class="h-12 bg-white/5 rounded-xl border border-white/5 flex items-center justify-center grayscale opacity-30 hover:grayscale-0 hover:opacity-100 transition-all cursor-pointer group">
                <Heart :size="16" class="group-hover:text-accent-gold" />
              </div>
              <div class="h-12 bg-white/5 rounded-xl border border-white/5 flex items-center justify-center grayscale opacity-30 hover:grayscale-0 hover:opacity-100 transition-all cursor-pointer group">
                <ShieldCheck :size="16" class="group-hover:text-accent-gold" />
              </div>
            </div>
          </div>
        </div>
