...
const navigationItems = [
  { name: 'Dashboard', path: '/sponsorship/admin', icon: LayoutDashboard },
  { name: 'Sponsors CRM', path: '/sponsorship/admin/organizations', icon: Building2 },
  { name: 'Opportunities', path: '/sponsorship/admin/opportunities', icon: Target },
  { name: 'Sponsorship Deals', path: '/sponsorship/admin/sponsorships', icon: FileSignature },
  { name: 'Payments Console', path: '/sponsorship/admin/payments', icon: CreditCard },
  { name: 'Fulfillment', path: '/sponsorship/admin/fulfillment', icon: CheckSquare },
  { name: 'Reports & Analytics', path: '/sponsorship/admin/reports', icon: BarChart3 },
];

const isCurrentRoute = (path: string) => {
  if (path === '/sponsorship/admin') return route.path === '/sponsorship/admin';
...
const handleLogout = async () => {
  try {
    ...
    toast.success('Signed out from SPMS.');
    void router.push('/');
  } catch (error) {
    void router.push('/');
  }
      ...
      <div class="flex-1 flex flex-col min-h-0">
        ...
        <div class="p-6 border-b border-neutral-ivory/50 flex items-center justify-between">
          <router-link to="/" class="flex items-center gap-3 group">
            <div class="h-10 w-10 bg-primary text-white rounded-2xl flex items-center justify-center font-bold text-sm shadow-md group-hover:scale-105 transition-transform">
              <Briefcase class="w-5 h-5 text-white" />
            </div>
            <div>
              <h2 class="text-sm font-bold text-neutral-black tracking-wide group-hover:text-primary transition-colors">MSA Partnerships</h2>
              <span class="text-[10px] text-neutral-muted font-mono block">SPMS Shell v1.0</span>
            </div>
          </router-link>
    ...
    <div class="flex-1 flex flex-col min-w-0">
      ...
      <header class="lg:hidden bg-white border-b border-neutral-ivory px-4 py-3 flex items-center justify-between sticky top-0 z-30">
        <div class="flex items-center gap-3">
          ...
          <span class="font-bold text-neutral-black text-sm">Partnerships (SPMS)</span>
        </div>
