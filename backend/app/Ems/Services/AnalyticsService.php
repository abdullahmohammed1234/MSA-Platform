...
class AnalyticsService
{
    ...
    private function computeTicketPerformance(array $eventIds): array
    {
        ...
        foreach ($ticketTypes as $type) {
            ...
            // Revenue generated by this ticket type
            $revenue = (float) Payment::where('status', PaymentStatus::Paid->value)
                ->whereIn('registration_id', function ($query) use ($type) {
                    $query->select('id')->from('ems_registrations')->where('ticket_type_id', $type->id);
                })
                ->sum('amount');
